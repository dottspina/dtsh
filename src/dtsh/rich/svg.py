...
class SVGFormat:
    ...
    * The terminal output GBox
    ...
    # Vertical space from the viewBox top down to the bottom
    # of the macOS-like buttons when present.
    VSPAN_DOWN_GCIRCLES: int = 29

    # Capture output default vertical coordinate: we'll compensate
    # for the title bar only if this matches the coordinate
    # of the captured output of the parsed SVG document.
    TERM_Y_DEFAULT = 41
...
class SVGDocument:
    ...
    # The SVG we're appending to this one (append mode).
    _svg_next: Optional["SVGDocument"] = None
    ...
    def __init__(
        ...
        if not (self._show_gcircles or self._has_title):
            # No title bar: resize views.
            self.viewbox.set_width_height(
                self.viewbox.width,
                self.viewbox.height - SVGFormat.VSPAN_DOWN_GCIRCLES,
            )
            self.rect.set_width_height(
                self.rect.width,
                self.rect.height - SVGFormat.VSPAN_DOWN_GCIRCLES,
            )
            if self.gterminal.y == SVGFormat.TERM_Y_DEFAULT:
                # Translate capture output only if we're parsing SVG text
                # generated by the rich library (not one we've formatted
                # and to which we're appending).
                self.gterminal.set_xy(
                    self.gterminal.x,
                    self.gterminal.y - SVGFormat.VSPAN_DOWN_GCIRCLES,
                )
    ...
    def content(self) -> SVGText:
        ...
        if self._svg_next:
            # We're appending a captured command output
            # to an existing SVG file.

            # Put all base CSS into a single <style>.
            self.style.append(self._svg_next.style)
            # Put all clipping paths into a single <defs>.
            self.defs.append(self._svg_next.defs)

            # Stretch dimensions.
            rect_width: int = max(self.rect.width, self._svg_next.rect.width)
            rect_height: int = self.rect.height + self._svg_next.rect.height

            viewbox_width: int = max(
                self.viewbox.width, self._svg_next.viewbox.width
            )
            viewbox_height: int = (
                self.viewbox.height + self._svg_next.viewbox.height
            )

            # Translate the captured output we're appending.
            next_term_x = self.gterminal.x
            next_term_y = self._svg_next.gterminal.y + self.viewbox.height

            if self._show_gcircles or self._has_title:
                # Compensate for the missing title bar.
                viewbox_height -= SVGFormat.VSPAN_DOWN_GCIRCLES
                rect_height -= SVGFormat.VSPAN_DOWN_GCIRCLES
                next_term_y -= SVGFormat.VSPAN_DOWN_GCIRCLES

            # Update geometry.
            self.viewbox.set_width_height(viewbox_width, viewbox_height)
            self.rect.set_width_height(rect_width, rect_height)
            self._svg_next.gterminal.set_xy(next_term_x, next_term_y)

        # Assemble fragments into SVG.
        svg_txt: SVGText = []
        ...
        if self._svg_next:
            svg_txt.extend(self._svg_next.gterminal.content)
        SVGFragment.newline(svg_txt)

        svg_txt.append("</svg>")
        return svg_txt

    def append(self, svg_doc: "SVGDocument") -> None:
        """Append another SVG doument to this."""
        self._svg_next = svg_doc
